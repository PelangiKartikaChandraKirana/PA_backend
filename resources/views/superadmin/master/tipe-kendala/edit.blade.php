...
<div class="px-6 py-8 max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Jenis Kendala</h1>
            <p class="text-sm text-slate-500 mt-0.5">Ubah data referensi kendala mesin/absensi.</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <form action="{{ route('superadmin.master.tipe-kendala.update', $tipe_kendala->id) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="name" value="{{ old('name', $tipe_kendala->name) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="number" name="priority" value="{{ old('priority', $tipe_kendala->priority) }}" required min="1"
                           class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all">
                ...
                <div class="space-y-2">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" @checked(old('is_active', $tipe_kendala->is_active))>
