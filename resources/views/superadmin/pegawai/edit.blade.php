...
<div class="px-6 py-6">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Edit Pegawai</h1>
            <p class="mt-1 text-sm font-medium text-slate-500">Perbarui informasi data pegawai <b>{{ $pegawai->name }}</b>.</p>
        </div>
    ...
    <div class="max-w-4xl">
        <form action="{{ route('superadmin.pegawai.update', $pegawai->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div class="p-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="department_id" id="department_id" 
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept->id }}" {{ old('department_id', $pegawai->department_id) == $dept->id ? 'selected' : '' }}>
                                    {{ $dept->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="employee_type_id" id="employee_type_id" 
                            ...
                            @foreach($employeeTypes as $type)
                                <option value="{{ $type->id }}" {{ old('employee_type_id', $pegawai->employee_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="tpp_allowance" id="tpp_allowance" value="{{ old('tpp_allowance', $pegawai->tpp_allowance) }}"
                               class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm shadow-sm transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none"
